...
def paste_card_art(name, image, cost, temple):
    ...
    if not card_art:
        try:
            ...
            card_art = Image.open(f"assets/card_art/default.png").convert("RGBA")
...
def write_name(image, draw, name):
    written_name = name.replace("_alt", "")
...
def write_flavor_text(draw, font, flavor_text, temple):
    ...
    if flavor_text != "BLANK":
        ...
        draw.text((flavor_text_x, config['flavor_text_top_height']), flavor_text, fill=TEXT_COLORS[temple], font=font)
...
def draw_text(image, name, tier, temple, tribes, flavor_text, artist):
    ...
    # Print artist
    artist_text = f"Art by {artist}"
    desc_y = config['artist_bottom_height']
    desc_x = (image.width - draw.textlength(artist_text, font=heavyweight_font)) // 2
    draw.text((desc_x, desc_y), artist_text,
              fill=(164, 89, 62) if temple == "Terrain" and tier in ["Rare", "Talking"] else TEXT_COLORS[color_temple],
              font=heavyweight_font)
