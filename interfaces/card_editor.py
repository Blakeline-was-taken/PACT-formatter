...
class NoWheelComboBox(QComboBox):
    ...
    def wheelEvent(self, event):
        """Ignore wheel changes so scrolling a form cannot alter this field."""
        event.ignore()
...
class Gemification(QWidget):
    ...
    def __init__(self, with_text):
        """Create gem toggles, optionally including an effect text field."""
        """Create gem toggles, optionally including gemification effect text."""
        super().__init__()
        layout = QHBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)
        if with_text:
